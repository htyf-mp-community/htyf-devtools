...
const styles = {cssText: `
  .htyf-welcome{--brand:#d9483b;--brand-hover:#bd352c;--amber:#e99317;--green:#18864b;width:100%;height:100%;min-width:0;min-height:0;display:grid!important;place-items:safe center;padding:32px 24px;box-sizing:border-box;background:radial-gradient(circle at 50% 44%,color-mix(in srgb,var(--brand) 5%,transparent),transparent 42%),var(--color-background);overflow:auto}
  .htyf-card{width:100%;max-width:760px;border:1px solid color-mix(in srgb,var(--sys-color-divider) 82%,transparent);border-radius:18px;background:var(--sys-color-base);color:var(--color-text-primary);box-shadow:0 24px 64px rgba(25,31,40,.10),0 2px 8px rgba(25,31,40,.04);overflow:hidden}
  ...
  .htyf-title-row{display:flex;align-items:center;flex-wrap:wrap;gap:4px 16px}.htyf-title-row .htyf-title{flex-shrink:0}.htyf-description-row{display:flex;align-items:center;flex-wrap:wrap;gap:8px 12px;margin-top:6px}.htyf-description-row .htyf-description{margin:0}
  ...
  @media(max-width:620px){.htyf-welcome{padding:16px}.htyf-header{padding:20px;align-items:flex-start}.htyf-content{padding:20px;gap:20px}.htyf-website{font-size:11px}.htyf-website-feedback{margin:0 20px 12px}.htyf-footer{padding:14px 20px}.htyf-chips{display:none}.htyf-content{grid-template-columns:1fr}.htyf-qr-panel{text-align:left}.htyf-guide{flex-wrap:wrap}.htyf-footer{align-items:flex-start;flex-direction:column;gap:5px}.htyf-footer span{white-space:normal}}@media(prefers-color-scheme:dark){.htyf-card{box-shadow:0 20px 56px rgba(0,0,0,.28)}.htyf-mark,.htyf-step{background:rgba(217,72,59,.16)}.htyf-qr-frame{box-shadow:none}}@media(prefers-reduced-motion:reduce){.htyf-copy,.htyf-update,.htyf-website-link{transition:none}}
...
class HTYFWelcome extends UI.Widget.VBox {
  ...
  #render() {
    ...
    this.contentElement.querySelector('.htyf-website-link').addEventListener('click', event => {
  ...
  async #openWebsite(link) {
    if (link.getAttribute('aria-disabled') === 'true') return;
    const feedback = this.contentElement.querySelector('.htyf-website-feedback');
    link.setAttribute('aria-disabled', 'true');
    link.setAttribute('aria-busy', 'true');
    feedback.textContent = '正在打开浏览器…';
    try {
      await window.devtoolsHost.openWebsite();
      feedback.textContent = '已请求在默认浏览器中打开官网。';
    } catch {
      feedback.textContent = '未能打开浏览器，请重试，或在浏览器中访问 https://mp.dagouzhi.com/';
    } finally {
